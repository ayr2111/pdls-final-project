...
# data
parser.add_argument('--data_dir', type=str, default='../../CholecT45', help='path to dataset?')
...
parser.add_argument('-l', '--initial_learning_rates', type=float, nargs='+', default=[0,0.01, 0.01, 0.01], help='List learning rates for tool, verb-target, triplet respectively')
...
def train_loop(dataloader, model, activation, loss_fn_i, loss_fn_v, loss_fn_t, loss_fn_ivt, optimizers, scheduler, epoch):
    ...
    for sch in scheduler:
        sch.step()
    wandb.log({"loss_i": loss_i.item(), "loss_v": loss_v.item(), "loss_t": loss_t.item(), "loss_ivt": loss_ivt.item()})
...
module_i        = list(set(model.parameters()) - set(model.encoder.cagam.parameters()) - set(model.decoder.parameters()))
module_vt       = model.encoder.cagam.parameters()
module_ivt      = model.decoder.parameters()
